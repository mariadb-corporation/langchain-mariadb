...
try:
    from griffe import load, Docstring, Module, Class, Function, Attribute
except ImportError:
...
def render_docstring_section(section: Any) -> str:
    ...
    if section.kind.value == "text":
        ...
        if text and not any(keyword in text for keyword in ["Args:", "Returns:", "Raises:", "Examples:", "Attributes:", "Note:"]):
            lines.append(text)
            lines.append("")
        ...
        if isinstance(section.value, list):
            for ret in section.value:
                ...
                if annotation or desc:
                    lines.append(f"{annotation} - {desc}" if annotation and desc else (annotation or desc))
        else:
            annotation = f" `{section.value.annotation}`" if section.value and section.value.annotation else ""
...
def document_class(cls: Class, level: int = 2) -> str:
    ...
    if cls.docstring:
        ...
        if cls.docstring.value:
            summary = cls.docstring.value.split('\n\n')[0].split('Args:')[0].split('Example:')[0].strip()
            if summary:
                lines.append(summary)
                lines.append("")
    ...
    if "__init__" in cls.members:
        ...
        if init.docstring:
            ...
            if init.docstring.value:
                summary = init.docstring.value.split('\n\n')[0].split('Args:')[0].strip()
                if summary:
                    lines.append(summary)
                    lines.append("")
    ...
    # Public methods
    methods = [
        (name, member) for name, member in cls.members.items()
...
def main():
    ...
    modules = {
        ...
        'chat_message_histories.md': ('langchain_mariadb.chat_message_histories', 'Chat Message History'),
        'expression_filter.md': ('langchain_mariadb.expression_filter', 'Expression Filters'),
